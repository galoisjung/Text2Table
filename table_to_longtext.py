...
- 표가 이미 Markdown 문자열(table_markdown 필드)로 와 있으므로, 별도 그룹핑/렌더링
  없이 그대로 프롬프트에 삽입한다. JSON은 어디까지나 구조화된 메타데이터(table_id,
  context 등)를 담는 컨테이너일 뿐, LLM 프롬프트에는 table_markdown(순수 텍스트)만
  들어간다.
...
def main():
    parser = argparse.ArgumentParser(
        description="pdf_table_extractor.py의 *_tables.json을 읽어 표를 장문 서술로 변환합니다."
    )
